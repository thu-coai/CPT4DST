    ...
    for function in text.split(") "):
        if(function!=""):
            if "(" in function:
                ...
                function = function[startpos+1:]
                # intent, parameters = function.split("(")
                parameters = sum([s.split('",') for s in function.split("=")],[])
                if len(parameters)>1:
                    if len(parameters) % 2 != 0:
                        parameters = parameters[:-1]

                    for i in range(0,len(parameters),2):
                        API[parameters[i]] = parameters[i+1].replace('"',"")
    ...
    for p, g in zip(pred,gold):
        ...
        if(g!=""):
            ...
            if(p!="" and "(" in p and ")"): ## means the predicted text is an API
                ...
                state_G = {s:v for s,v in API_G.items() if s !="none"}
                state_P = {s:v for s,v in API_P.items() if s !="none"}

                # print(json.dumps(state_G, indent=4))
                # print(json.dumps(state_P, indent=4))
                # print(diff(state_G, state_P))
                # print('='*50)
...
def score_folder(folder=None, args=None, time='FINAL'):
    if not args:
    ...
    # names = args.dataset_order
    RESULT = []
    for folder in folders:
        if "png" in folder or "TOO_HIGH_LR" in folder or "TEMP" in folder:
            continue
        res = evaluate(args,f'{folder}/{time}/generated_responses.json')#, ent=entities_json)
        if(args.task_type == "INTENT"):
            ...
            RESULT.append({"Name":folder.split("/")[-1],"JGA":JGA, "DOMAIN_ACC": res["ACC"]})
            for k,v in res["API"].items():
                RESULT.append({"Name":k,"JGA":v, "DOMAIN_ACC": 0})
    ...
    print(tabulate(RESULT, headers="keys", tablefmt="github"), file=open(f'{folder}/{time}/result.txt', 'w'))
